...
export default function SucursalesPage() {
  ...
  const fetchInventarioSucursal = useCallback(async (id_sucursal: number): Promise<InventarioItem[]> => {
    try {
      const res = await fetch(`${API_URL}/inventario?sucursal_id=${id_sucursal}`, { credentials: 'include' });
      if (!res.ok) return [];
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [resSucursales, resProductos] = await Promise.all([
        fetch(`${API_URL}/inventario/sucursales`, { credentials: 'include' }),
        fetch(`${API_URL}/productos?page=1&limit=100`, { credentials: 'include' }),
      ]);
  ...
  const handleDeleteProducto = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${API_URL}/productos/${deleteTarget}`, { method: 'DELETE', credentials: 'include' });
      if (!res.ok) {
  ...
  const handleDeleteSucursal = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/sucursales/${deleteSucursalTarget.id}`, { method: 'DELETE', credentials: 'include' });
      if (!res.ok) {
        const d = await res.json();
