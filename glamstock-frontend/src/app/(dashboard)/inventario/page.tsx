...
export default function InventarioPage() {
    ...
    try {
      ...
      const inventarios = await Promise.all(
        listaSucursales.map(async (s) => {
          const r = await fetch(`${API_URL}/inventario?sucursal_id=${s.id_sucursal}`, { credentials: 'include' });
          if (!r.ok) return [];
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`${API_URL}/productos/${deleteId}`, { method: 'DELETE', credentials: 'include' });
      if (!res.ok) {
