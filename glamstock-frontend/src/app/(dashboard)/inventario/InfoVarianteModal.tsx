...
export default function InfoVarianteModal({ open, varianteId, inventarioId, onClose }: InfoVarianteModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`${API_URL}/variantes/${varianteId}`, { credentials: 'include' });
        ...
        if (v.id_producto_maestro) {
           const pRes = await fetch(`${API_URL}/productos/${v.id_producto_maestro}`, { credentials: 'include' });
           if (pRes.ok) {
        ...
        let invStock = 0;
        if (inventarioId) {
           const resInv = await fetch(`${API_URL}/inventario/${inventarioId}`, { credentials: 'include' });
           if (resInv.ok) {
